...
@FeignClient(name = "quran-api", url = "${api.quranapi.host}", configuration = {QuranApiClientConfiguration.class,QuranApiErrorDecoder.class})
public interface QuranApiClient {

    @GetMapping("/v1/surah/{surah}/{edition}")
    SurahResponseDto findSurahByChapterAndEdition(@PathVariable("surah") Integer surah, @PathVariable("edition") String edition);
}
