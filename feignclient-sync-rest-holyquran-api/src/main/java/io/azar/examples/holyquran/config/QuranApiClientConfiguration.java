...
public class QuranApiClientConfiguration {
    ...
    @Bean
    public Client feignClient(PoolingHttpClientConnectionManager poolingHttpClientConnectionManager)
        throws Exception {
        List<X509Certificate> x509Certificates = CertUtils.loadCertificates(
            apiConfigurationProperties.getQuranapi().getTruststore().getCertificates());
...
//
        PoolingHttpClientConnectionManager connectionManager =
            new PoolingHttpClientConnectionManager(10000, TimeUnit.MILLISECONDS);
        ...
        connectionManager.closeIdleConnections(30,
            TimeUnit.SECONDS); // Close idle connections after 30 seconds
        new PoolingHttpClientConnectionManagerMetricsBinder(connectionManager,
            "quran-http-conn-pool").bindTo(meterRegistry);
